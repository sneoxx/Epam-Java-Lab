...
import com.zaraev.epam.pojo.HobbyPojo;
...
public class ReflectionService {
  ...
  public boolean checkHobbyAnnotation(Object object) {
    Class<?> clazz = object.getClass();
    if (clazz.isAnnotationPresent(Hobby.class)) {
      log.trace("checkEntityAnnotation() - Hobby аннотация найдена");
      return true;
    }
    log.trace("checkEntityAnnotation() - Hobby аннотация не найдена");
    return false;
  }

  //Демонстрация получения информации из анннотации и генерация с помощью нее сущности
  public void myHobbyAnnotationDemonstration() {
    NationalGeographicChannel nationalGeographicChannel = new NationalGeographicChannel();
    if (nationalGeographicChannel.getClass().isAnnotationPresent(Hobby.class)) {
      log.trace("Найдена @Hobby аннотация: {} на классе: {}"
          , nationalGeographicChannel.getClass().getAnnotation(Hobby.class)
          , nationalGeographicChannel.getClass());

      try {
        //Создаем объект с помощью рефлексии
        Class<?> clazz = Class.forName(HobbyPojo.class.getName());
        HobbyPojo myReflectionObject = (HobbyPojo) clazz.getDeclaredConstructor().newInstance();

        //Получаем аннотацию с помощью рефлексии
        Hobby myAnnotation = nationalGeographicChannel.getClass().getAnnotation(Hobby.class);

        //сетаем информацию, полученную из аннотации в объект
        myReflectionObject.setTypeOfContent(myAnnotation.kind());
        myReflectionObject.setMaterialDifficultyLevel(myAnnotation.level());
        List<String> list = new ArrayList<>(Arrays.asList(myAnnotation.tags()));
        myReflectionObject.setKindOfHobby(list);
        myReflectionObject.setChannelOwner(myAnnotation.okStringProp());
        myReflectionObject.setSource(myAnnotation.description().name());
        //сетаем приватное поле объекта
        Field field = HobbyPojo.class.getDeclaredField("privateField");
        field.setAccessible(true);
        field.set(myReflectionObject,
            "Сетаем приватное поле через рефлексию");
        log.trace("На основе информации из аннотации с помощью рефлексии сгенерирован объект: {}", myReflectionObject);

      } catch (ClassNotFoundException | InstantiationException | NoSuchMethodException
          | NoSuchFieldException | InvocationTargetException | IllegalAccessException e) {
        log.trace("myHobbyAnnotationDemonstration() ", e);
      }
    }
  }
   ...
   *
   * @param object - экземпляр класса
   * @return - true в случае наличия Entity на классе и Value на всех полях или методах класса, иначе false в случае
   * наличия Entity на классе и Value не всех полях класса выкинет NoValueAnnotationException в случае отстутсвия Entity
   * на классе и пристутствия Value хотя бы на части полей выкинет IllegalStateException
   */
  public boolean checkValueFieldsAndMethodsAnnotations(Object object) {
    if (checkEntityAnnotation(object)) {
      ...
      if (checkValueFieldsAnnotation(object) || checkValueMethodsAnnotation(object)) {
        log.error(
            "checkValueFieldsAndMethodsAnnotations() - Entity аннотация не найдена, хотя Value аннотации присутствуют");
  ...
  /**
   * Проверка наличия аннотации Value в полях и методах класса и последующая передача на заполнение полей аннотацией
   * Value с обработкой возможных исключений
   *
   * @param object          - экземпляр класса
  ...
  public void checkAndFillFieldsWithValueAnnotation(Object object, Object valueAnnotation) {
    log.info(
        "checkAndFillFieldsWithValueAnnotation() - На заполнение в объект: {} передано значение valueAnnotation: {}",
        object.getClass().getTypeName(), valueAnnotation);
    try {
  ...
  public void fillFieldsWithValueAnnotationDirectly(Object object, Object valueAnnotation) {
    ...
    for (Field field : declaredFields) {
      if (field.isAnnotationPresent(Value.class)) {
        if (Objects.nonNull(valueAnnotation) && field.getType().getName().replaceFirst("int", "java.lang.Integer")
            .equals(valueAnnotation.getClass().getTypeName())) {
          setFieldValueAnnotation(object, valueAnnotation, field);
          log.info("fillFieldsWithValueAnnotationDirectly() - Успешное заполнение переданным значением: {}",
              valueAnnotation);
        } else {
          log.debug(
              "fillFieldsWithValueAnnotationDirectly() - Переданный тип значения Value не соответствует полю класса: {}",
              field.getName());
          ...
          if (Objects.nonNull(defaultValueAnnotation)) {
            if (field.getType().getName().replaceFirst("int", "java.lang.Integer").equals("java.lang.Integer")) {
              ...
              log.info("fillFieldsWithValueAnnotationDirectly() - Успешное заполнение поля: {} дефолтным значением: {}",
                  field.getName(), value);
              ...
              setFieldValueAnnotation(object, value, field);
              log.info(
                  "fillFieldsWithValueAnnotationDirectly() - Успешное заполнение поля: {} дефолтным значением: {}}",
                  field.getName(), value);
            }
          }
        }
    ...
    for (Method method : declaredMethods) {
      if (method.isAnnotationPresent(Value.class)) {
        ...
        if (Objects.nonNull(valueAnnotation) && parameterType.equals(valueAnnotation.getClass().getTypeName())) {
          ...
          log.info("fillFieldsWithValueAnnotationViaSetter() - Успешное заполнение переданным значением: {} ",
              valueAnnotation);
        } else {
          log.debug(
              "fillFieldsWithValueAnnotationViaSetter() - Переданный тип значения Value не соответствует входному параметру метода: {}",
              method.getName());
  ...
  public void setFieldValueAnnotation(Object object, Object value, Field field) {
    ...
    try {
      ...
      log.info("setFieldValueAnnotation() - Поле: {} заполнено значением Value: {}", field.getName(),
          field.get(object));
  ...
  public void setFieldViaSetterValueAnnotation(Object object, Object value, Method method) {
    try {
      ...
      log.debug("setFieldViaSetterValueAnnotation() - В метод: {} отправлено значение Value: {}", method.getName(),
          value);
